...
/*TABLA BASE_ESC SIRVE PARA OBTENER EL PERIODO EN EL CUAL UN CLIENTE MIGRÓ AL NUEVO SISTEMA DE ORACLE*/
/*BASE_ESC TABLE IS USED TO OBTAIN THE PERIOD IN WHICH A CLIENT MIGRATED TO THE NEW ORACLE SYSTEM*/
WITH BASE_ESC AS ( 
...
/*LA TABLA BASE CONSOLIDA LA INFORMACIÓN PROVENIENTE DE LAS DISTINTAS FUENTES DE ORIGEN*/
/*THE BASE TABLE CONSOLIDATES THE INFORMATION FROM THE DIFFERENT SOURCES*/
BASE AS (
SELECT 
  A.PERIODO, 
...
/* BASE2 CLASIFICA FUENTES, TIPIFICA CARGOS Y ENRIQUECE LA INFORMACIÓN DEL CLIENTE */
/* BASE2 CLASSIFIES SOURCES, TYPIFIES CHARGES AND ENRICHES CUSTOMER INFORMATION */
BASE2 AS (
SELECT 
  A.PERIODO, 
  A.FLAG_FUENTE,
...
FROM BASE A
  ...
  LEFT JOIN (SELECT * FROM CGE_TBSEGMENTO_CIERRE WHERE PERIODO = '202601') E
...
/* TMP_REVISION_CLIENTE AGREGA LA COLUMNA RENTA CALCULADA A PARTIR DE BASE2 */
/* TMP_REVISION_CLIENTE ADDS THE CALCULATED RENT COLUMN FROM BASE2 */
SELECT 
  A.*,
...
/*LA TABLA BASE_VAR_RATIO_A SIRVE PARA OBTENER EL MONTO DEL DESCUENTO Y DEL INGRESO DE UN CLIENTE EN EL MES*/
/*THE TABLE BASE_VAR_RATIO_A IS USED TO OBTAIN THE DISCOUNT AMOUNT AND INCOME OF A CUSTOMER IN THE MONTH*/
...
/*LA TABLA BASE_VAR_RATIO_B ES PARA OBTENER EL RATIO DESCUENTO/INGRESO */
/*THE BASE_VAR_RATIO_B TABLE IS FOR OBTAINING THE DISCOUNT/INCOME RATIO */
BASE_VAR_RATIO_B AS (
SELECT DISTINCT
  PERIODO,
  RUCCOMPANIA,
...
/*LA TABLA BASE_VAR_RATIO_C OBTIENE LOS RATIOS DE LOS ÚLTIMOS 6 MESES, POR LO QUE CADA MES SE TIENE QUE HACER UN AJUSTE MANUAL*/
/*THE BASE_VAR_RATIO_C TABLE OBTAINS THE RATIOS FROM THE LAST 6 MONTHS, SO A MANUAL ADJUSTMENT MUST BE MADE EACH MONTH*/
BASE_VAR_RATIO_C AS (
SELECT 
  RUCCOMPANIA,
...
/*LA BASE BASE_VAR_RATIO_D UBICA CUATRO DIFERENCIAS MENSUALES DE LOS RATIOS*/
/*THE BASE_VAR_RATIO_D LOCATES FOUR MONTHLY DIFFERENCES IN THE RATIOS*/
BASE_VAR_RATIO_D AS (
SELECT
  RUCCOMPANIA,
...
/*LA TABLA BASE_VAR_RATIO_E OBTIENE LA MAYOR DIFERENCIA ABSOLUTA*/
/*THE TABLE BASE_VAR_RATIO_E OBTAINS THE GREATEST ABSOLUTE DIFFERENCE*/
BASE_VAR_RATIO_E AS (
SELECT
  RUCCOMPANIA,
...
/*LA TABLA BASE_VAR_RATIO_F AGRUPA LAS DIFERENCIAS PARA UN MEJOR ANALISIS*/
/*THE BASE_VAR_RATIO_F TABLE GROUP THE DIFFERENCES FOR BETTER ANALYSIS*/
BASE_VAR_RATIO_F AS (
SELECT
  RUCCOMPANIA,
  --MOM_RATIO_MAX,
...
/* ANALISIS_A AGREGA INFORMACIÓN FINANCIERA Y ESTADOS DEL SUSCRIPTOR PARA EL ANÁLISIS DE CLIENTES A NIVEL PERIODO */
/* ANALYSIS_A ADDS SUBSCRIBER FINANCIAL INFORMATION AND STATEMENTS FOR CUSTOMER ANALYSIS AT THE PERIOD LEVEL */
ANALISIS_A AS (
SELECT 
  A.PERIODO, 
  A.FLAG_FUENTE, 
...
/* ANALISIS_B CONSOLIDA Y DISTRIBUYE LA RENTA POR CLIENTE EN COLUMNAS MENSUALES */
/* ANALYSIS_B CONSOLIDATES AND DISTRIBUTES THE INCOME PER CLIENT IN MONTHLY COLUMNS */
ANALISIS_B AS (
SELECT
  --PERIODO, 
...
/*ANALISIS_C AÑADE LA INFORMACIÓN TOTAL 2025, VARIACIÓN MES ACTUAL CONTRA MES PASADO Y SEÑALA SI UN CLIENTE FACTURÓ EN EL MES*/
/*ANALYSIS_C ADDS TOTAL INFORMATION FOR 2025, CURRENT MONTH VARIATION COMPARED TO PREVIOUS MONTH, AND INDICATES IF A CUSTOMER INVOICED IN THE MONTH*/
ANALISIS_C AS (
SELECT
  A.*,
...
/*LA TABLA FINAL PERMITE REVISAR LAS DIFERENCIAS DE RATIO DESCUENTO POR CADA CLIENTE PARA EVALUAR SI ES QUE SE LE DIÓ DESCUENTOS DE MÁS O NO SE LES ASIGNÓ ALGUNO.
NOS PERMITE REVISAR POR CATEGORÍAS, CLIENTE, CLIENTES ESPECIALES Y MAYORES DIFERENCIAS DE RATIOS*/
